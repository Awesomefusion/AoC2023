...
def parse_input(data):
    ...
    vert_sym = [[sym.start() for sym in re.finditer(r'[^\.]', ''.join(line))] for line in
                zip(*data_list)]  # Use zip to get columns
...
def light_beam(point, m, bool_map, direction):
    ...
    if value is not None:
        ...
        for i in get_range_edge(direction, point[1], len(
                bool_map[point[0]])):  # You've hit an edge, let the program know to close this branch
            if direction in 'LR':
                bool_map[point[0]][i] = True
            else:
                bool_map[i][point[0]] = True
...
def check_all_options(data):
    ...
    for i in range(0, length):
        directions = [(i, -1, 'R'), (i, length, 'L'), (-1, i, 'D'),
                      (length, i, 'U')]  # These are grouped into x value, y value, and direction
        for entry in directions:
            l, h, v = copy.deepcopy(data)  # Make a copy since you are going to need a fresh copy each time
            l, mirror_hit = move(l, h, v, (entry[0], entry[1]), entry[2],
